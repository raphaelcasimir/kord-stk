...
%		The angle to position was taken out. if put back, change the intro
		...
		\explain{$F_{Pa}$ is the parallel force resulting from the decomposition of the thruster force}{\si{\newton}}
		\explain{$F_{Pe}$ is the perpendicular force resulting from the decomposition of the thruster force}{\si{\newton}}
		\stopexplain
		
		\subsection{Modeling of the Angle}
...
%		Let's call $l_{Cg}$ the distance between the center of gravity and the end of the thruster.
		...
		\SI{70}{\percent} of the weight is in fact concentrated on the electronic's stage (rocket is about \SI{0.300}{\kilogram} and the electronics stage is around \SI{0.210}{\kilogram}). Therefore it is assumed, that apart from this stage everything is weightless. Consequently the rocket can be approximated as a pendulum where the electronics stage is mass rotating around the gravity center. The center of gravity position is determined by simply balancing the rocket on a finger \cite{web:CenterGravityDet}. The moment of inertia is then found in \autoref{eq:rocket_angle_3} \cite{web:PendulumMomentOfInertia}.
		
		\begin{equation} \label{eq:rocket_angle_3}
		I_r=m_{Es} l_{Es}^2 \addunit{\si{\kilogram\meter\squared}}
		\end{equation}
		\startexplain
...
%		\stopexplain
		...
		\begin{equation}\label{eq:UnlinearRockAngle}
			m_{Es} l_{Es}^2 \ddot{\theta}_b = F_t \sin(\theta_t) l_{Cg} \addunit{\si{\newton\meter}}
		\end{equation}
		
		The thruster force $F_t$ will be considered as constant. See the rocket motor test experiment for details in \autoref{ThrusterTest}.
		...
		\begin{equation}\label{eq:linearRockAngle}
			m_{Es} l_{Es}^2 \ddot{\theta}_b = F_t \theta_t l_{Cg} \addunit{\si{\newton\meter}}
		\end{equation}
		
		By going to the Laplace domain and grouping elements the transfer function of the rocket is found in \autoref{eq:RockTansFunc}.
		
		\begin{subequations} \label{eq:RockTansFunc}
			\begin{flalign}
				s^2 m_{Es} l_{Es}^2 \Theta_b(s)	&=F_t l_{Cg} \Theta_t(s) \addunit{1} \\
				\frac{\Theta_b(s)}{\Theta_t(s)}		&=\frac{F_t l_{Cg}}{s^2 m_{Es} l_{Es}^2} \addunit{1}
			\end{flalign}
		\end{subequations}
		...
		\frac{\Theta_{b}(s)}{\Theta_{b_{in}}(s)}=\frac{1}{\tau s + 1} \addunit{1}
		\end{equation}
		\startexplain
		\explain{$\Theta_{b_{in}(s)}$ is the angle sent to the servo by the controller}{1}
		...
		\begin{equation}\label{eq:RockTansFuncFinal}
			\frac{\Theta_b(s)}{\Theta_t(s)}=\frac{1}{\tau s + 1}\frac{F_t l_{Cg}}{s^2 m_{Es} l_{Es}^2}
		\end{equation}
		
		Now that the model of the rocket is done a comparison is done with the model of the inverted pendulum.
