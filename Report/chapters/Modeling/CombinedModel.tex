...
By combining \autoref{eq:UmThetaaTFSimplified} and \autoref{eq:tfArmStick} the model for the inverted pendulum becomes \autoref{eq:CombModel}.
\begin{flalign}
\frac{\Theta_s(s)}{U_m(s)}=\frac{\nicefrac{K_t}{R_m}}{\left( \frac{J_{m}+J_{gear}}{N^3} - J_a \right)s^2 + \left( \frac{\left( \frac{K_e K_t}{R_m} + B_{gear} + B_{m}\right)}{N^3} \right) s + gl_a(M_s+M_a)} \cdot \frac{-\frac{3l_a}{2l_s}s^2+\frac{3b_{as}}{M_sl_s^2}s}{s^2+\frac{3b_{as}}{M_sl_s^2}s-\frac{3g}{2l_s}} \label{eq:CombModel}
\end{flalign}
...
\frac{\Theta_s(s)}{U_m(s)}=\frac{-8.0725}{s^2-23.8185s-416.1281}
\end{flalign}
