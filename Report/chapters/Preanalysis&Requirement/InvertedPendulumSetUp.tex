...
\label{fig:ArmStick}
\end{figure}
\todo[inline,author=Jacob]{Add the reactionary forces where arm and stick connects.}
\startexplain
...
The system has a zero in 0 (two if the friction is considered negligible) which gives a 0 dB DC gain. This makes sense as the stick shouldn't move if the angle of the arm is constant. The poles show that the natural frequency of the system depends only on the gravity and length of the stick. This is similar to \autoref{eq:pendulum} for the frequency of a simple pendulum. \todo[inline,author=Jacob]{Make bib reference.}
