...
We now have the following rocket transfer function.

\begin{equation}
R = 5*10^3 * (s + 2) / (s + 1000) * (s + 50)/ (s + 1100) * 1 / (s * \uptau + 1) * F_t * L_{Cg} * 1 / (M_r * L_{Es}^2) / s^2
\end{equation}

\begin{figure}[htbp]
	\centering
	
	\includegraphics[width=\textwidth]{figures/Rocket/design/final_chart_tfcontroller}
