...
\begin{figure} [htbp] 
	\centering
	\includegraphics[width=\textwidth]{rlocusVArmGeneralPControllerStepResp.eps}
...
In order to speed up the inner loop two options are available. The first one is to use more elaborate controller than the P controller such as a PD controller or a lead. The second one is to once again divide the inner loop into two loops. As the second one is simpler to implement and has also the advantage to add a control over the velocity, this method is chosen. The two new loops are respectively the voltage as the input and the velocity which will be called motor loop from now on, and the velocity as the input and the arm's angle as the output named as arm's loop.

The first loop wich will be designed will be the motor loop as it is the most inner loop.

%\input{Design/MotorLoop.tex}

Now the arm's loop will be designed. The motor loop part has a settling time of \SI{0.0159}{\second}, so the settling time of the arm's loop has to be at least \SI{0.159}{\second} in order to make the assumption that the motor's loop transfer function is equal to 1 for the arm's loop.

The root locus of the new transfer function of the arm, in \autoref{fig:WArmLoopRLocus}, shows that there is theoretically no gain limitation for the arm as the pole moves to left side plan making the system stable with a gain high enough. The pole is also always on the real axis which means there will be no overshoot no matter the gain.
...
Since it is so our only limitation is the settling time os the motor as explained earlier. A gain of 905 is good enough as it gives a settling time of \SI{0.16}{\second} as seen in \autoref{fig:WArmLoopStepResp}.

\begin{figure} [htbp] 
	\centering
	\includegraphics[width=\textwidth]{WArmPControllerStepResp.eps}
